...
class SmartPlaylistsModel : public SimpleTreeModel<SmartPlaylistsItem> {
 ...
 public:
  SmartPlaylistsModel(CollectionBackend *backend, Application* app, QObject* parent = nullptr);
  ~SmartPlaylistsModel();
 ...
 protected:
  void LazyPopulate(SmartPlaylistsItem *item) { LazyPopulate(item, true); }
 ...
 private:
  ...
  void ItemFromSmartPlaylist(const QSettings &s, bool notify);
